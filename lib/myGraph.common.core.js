    myGraph = window.myGraph || {ismyGraph: true};
...
(function (win, doc, undefined)
{
    ...
    */
    ...
    RG.Registry.store['__myGraph_event_listeners__'] = []; // Used in the new system for tooltips
    RG.Background     = {};
    RG.background     = {};
    RG.objects        = [];
    RG.Resizing       = {};
    ...
    */
    ...
    RG.ISOLD  = myGraph.ISIE6 || myGraph.ISIE7 || myGraph.ISIE8; // MUST be here
    ...
    */
    ...
    RG.Register = function (obj)
    {
        ...
        if (!obj.Get('chart.noregister')) {
            ...
            myGraph.ObjectRegistry.Add(obj);
    ...
    */
    ...
    RG.Redraw = function ()
    {
        var objectRegistry = myGraph.ObjectRegistry.objects.byCanvasID;

        // Get all of the canvas tags on the page
        var tags = document.getElementsByTagName('canvas');

        for (var i=0,len=tags.length; i<len; ++i) {
            if (tags[i].__object__ && tags[i].__object__.ismyGraph) {
                
                // Only clear the canvas if it's not Trace'ing - this applies to the Line/Scatter Trace effects
                if (!tags[i].noclear) {
                    myGraph.clear(tags[i], arguments[0] ? arguments[0] : null);
                }
            }
        }
    ...
    RG.RedrawCanvas = function (ca)
    {
        ...
        */
        for (var i=0,len=objects.length; i<len; ++i) {
            if (objects[i]) {
                if (objects[i] && objects[i].ismyGraph) { // Is it an myGraph object ??
                    objects[i].Draw();
                }
            }
        }
...
/*
    ...
    RG.HideZoomedCanvas = function ()
    {
        ...
        if (prop['chart.zoom.fade.out']) {
            for (var i=frames,j=1; i>=0; --i, ++j) {
                if (typeof RG.zoom_image === 'object') {
                    setTimeout("myGraph.zoom_image.style.opacity = " + String(i / 10), j * interval);
                }
            }

            if (typeof RG.zoom_background === 'object') {
                setTimeout("myGraph.zoom_background.style.opacity = " + String(i / frames), j * interval);
            }
        }

        if (typeof RG.zoom_image === 'object') {
            setTimeout("myGraph.zoom_image.style.display = 'none'", prop['chart.zoom.fade.out'] ? (frames * interval) + 10 : 0);
        }

        if (typeof RG.zoom_background === 'object') {
            setTimeout("myGraph.zoom_background.style.display = 'none'", prop['chart.zoom.fade.out'] ? (frames * interval) + 10 : 0);
        }
    };
    ...
    */
    ...
    /**
    * Used to fire one of the myGraph custom events
    * 
    ...
    */
    myGraph.removeAllCustomEventListeners =
    myGraph.RemoveAllCustomEventListeners = function ()
    {
        var id = arguments[0];
    ...
    */
    ...
    RG.DrawBackgroundImage = function (obj)
    {
        ...
        if (typeof prop['chart.background.image'] === 'string') {
            if (typeof ca.__myGraph_background_image__ === 'undefined') {
                var img = new Image();
                img.__object__  = obj;
                img.__canvas__  = ca;
                img.__context__ = co;
                ...
                ca.__myGraph_background_image__ = img;
            } else {
                img = ca.__myGraph_background_image__;
            }

            // When the image has loaded - redraw the canvas
            img.onload = function ()
            {
                obj.__myGraph_background_image_loaded__ = true;
                RG.clear(ca);
                RG.redrawCanvas(ca);
            }
    ...
    */
    myGraph.ObjectRegistry.list =
    myGraph.ObjectRegistry.List = function ()
    {
        var list = [];
    ...
    */
    ...
    {
        var objects = myGraph.ObjectRegistry.objects.byUID;
    ...
    RG.InstallEventListeners = function (obj)
    {
        ...
        */
        if (RG.installCanvasClickListener) {
            ...
            //alert('[myGraph] You appear to have used dynamic features but not included the file: myGraph.common.dynamic.js');
        }
    };
    ...
    */
    RG.pr = function (obj)
    {
        ...
        switch (typeof obj) {
            ...
            case 'object':
                // In case of null
                if (myGraph.is_null(obj)) {
                    str += indent + 'null\n';
                } else {
                    str += indent + 'Object {' + '\n'
                    for (j in obj) {
                        str += indent + '    ' + j + ' => ' + myGraph.pr(obj[j], true, indent + '    ', counter + 1) + '\n';
                    }
                    str += indent + '}';
                }
                break;
    ...
    */
    ...
    /**
    * Uses the above myGraph.AJAX function but calls the call back passing an array as its argument.
    ...
    */
    ...
    RG.MeasureText = function (text, bold, font, size)
    {
        ...
        if (typeof myGraph.measuretext_cache === 'undefined') {
            myGraph.measuretext_cache = [];
        }

        var str = text + ':' + bold + ':' + font + ':' + size;
        if (typeof myGraph.measuretext_cache == 'object' && myGraph.measuretext_cache[str]) {
            return myGraph.measuretext_cache[str];
        }
        
        if (!myGraph.measuretext_cache['text-div']) {
            ...
            myGraph.measuretext_cache['text-div'] = div;

        } else if (myGraph.measuretext_cache['text-div']) {
            var div = myGraph.measuretext_cache['text-div'];
        }
        ...
        myGraph.measuretext_cache[str] = size;
    ...
    */
    ...
    {
        /**
        * An myGraph object can be given, or a string or the 2D rendering context
        * The coords are placed on the obj.coordsText variable ONLY if it's an myGraph object. The function
        ...
        */
        if (obj && obj.ismyGraph) {
            ...
            var co = ca.getContext('2d');
            var obj = ca.__object__;
        } else if (obj.toString().indexOf('CanvasRenderingContext2d') == -1 || myGraph.ISIE8 && obj.moveTo) {
            var co  = obj;
            var ca  = obj.canvas;
            var obj = ca.__object__;
        ...
        * if it's an myGraph object that was given)
        */
        if (obj && obj.ismyGraph && obj.coordsText) {
    ...
    * 
    * @param mixed  obj  This can either be the 2D context or an myGraph object
    ...
    */
    ...
    {
        /**
        * Allow either the myGraph object or the context to be used as the first argument
        */
        if (obj.ismyGraph && typeof obj.type === 'string') {
            var co = obj.context;
        } else {
            var co = obj;
        }
    ...
    */
    ...
    /**
    * Adds an event listener to myGraphs internal array so that myGraph can track them.
    ...
    */
    ...
    /**
    * Clears event listeners that have been installed by myGraph
    * 
    ...
    */
    ...
    RG.ClearEventListeners = function(id)
    {
        if (id && id == 'window') {
            ...
            window.removeEventListener('mousedown', window.__myGraph_mousedown_event_listener_installed__, false);
            window.removeEventListener('mouseup', window.__myGraph_mouseup_event_listener_installed__, false);
        
        } else {
            
            var canvas = document.getElementById(id);
            
            canvas.removeEventListener('mouseup', canvas.__myGraph_mouseup_event_listener_installed__, false);
            canvas.removeEventListener('mousemove', canvas.__myGraph_mousemove_event_listener_installed__, false);
            canvas.removeEventListener('mousedown', canvas.__myGraph_mousedown_event_listener_installed__, false);
            canvas.removeEventListener('click', canvas.__myGraph_click_event_listener_installed__, false);
        }
    };
    ...
    */
    ...
    /**
    * Sets an object in the myGraph registry
    * 
    ...
    */
    ...
    /**
    * Gets an object from the myGraph registry
    * 
    ...
    */
    ...
    RG.Reset = function (ca)
    {
        ...
        ca.__myGraph_aa_translated__ = false;
    ...
    * converts it to the myGraph stylee.
    * 
...
// End module pattern
    ...
    */
    window.$p  = function (v)
    {
        myGraph.pr(arguments[0], arguments[1], arguments[3]);
    };
